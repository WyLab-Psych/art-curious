      ...
      nested: {
        ...
        format: {
          ...
          nested: {
            ...
            // Multiple Choice
            mc_orientation: { type: ParameterType.STRING, pretty_name: "Multiple Choice Orientation", default: "vertical" },
            mc_options: { type: ParameterType.STRING, pretty_name: "Multiple Choice Options", array: true, default: [] },
            mc_write_in: { type: ParameterType.STRING, pretty_name: "Write-in Options", array: true, default: [] },
...
class WyLabSurveyPlugin implements JsPsychPlugin<Info> {
  ...
  trial(display_element: HTMLElement, trial: TrialType<Info>) {
    ...
    const xPath = "M55.1 73.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L147.2 256 9.9 393.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192.5 301.3 329.9 438.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.8 256 375.1 118.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192.5 210.7 55.1 73.4z";
    ...
    if (trial.custom_html) {
      ...
      for (let question_idx = 0; question_idx < question_order.length; question_idx++) {
        ...
        // MULTIPLE CHOICE INPUT
          ...
          for (let mc_option_idx = 0; mc_option_idx < mc.mc_options.length; mc_option_idx++) {
            const option_id = `${question_id}-opt-${mc_option_idx}`;
            html += `
              <label class="jspsych-survey-html-form-${question_format}-option-${mc_orientation}" for="${option_id}">
                ...
                <input type="${question_format}" name="${question_name}" id="${option_id}" value="${mc.mc_options[mc_option_idx]}" ${question_requirements}>
                <span class="${question_format}-button-label-${mc_orientation}">${mc.mc_options[mc_option_idx]}</span>`;
            
            if (mc.mc_write_in && mc.mc_write_in.includes(mc.mc_options[mc_option_idx])) {
      ...
      questions.forEach((q, index) => {
        if (q.requirements?.type === 'comprehension') {
          ...
          if (isCorrect) {
            ...
            svgPathElement?.parentElement?.setAttribute("viewBox", "-192 -256 768 1024");
